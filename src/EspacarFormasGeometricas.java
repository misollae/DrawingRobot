public class EspacarFormasGeometricas extends Comportamento {
	...
	public int getDistancia() {
		...
		if ((ultimo instanceof DesenharQuadrado && proximo instanceof DesenharQuadrado)
				|| (ultimo instanceof DesenharQuadrado && proximo instanceof DesenharPoligono
						&& ((DesenharPoligono) proximo).getNumLados() == 3)
				|| (ultimo instanceof DesenharPoligono
						&& ((DesenharPoligono) ultimo).getUltNumLados() == 3
						&& proximo instanceof DesenharQuadrado)) {
			return 0;
		}
		...
		if ((ultimo instanceof DesenharQuadrado && proximo instanceof DesenharCirculo)
				|| (ultimo instanceof DesenharPoligono
						&& ((DesenharPoligono) ultimo).getUltNumLados() == 3
						&& proximo instanceof DesenharCirculo)) {
		...
		if ((ultimo instanceof DesenharCirculo && proximo instanceof DesenharQuadrado)
				|| (ultimo instanceof DesenharCirculo && proximo instanceof DesenharPoligono
						&& ((DesenharPoligono) proximo).getNumLados() == 3)) {
		...
		if (ultimo instanceof DesenharCirculo && proximo instanceof DesenharCirculo) {
			return (((DesenharCirculo) ultimo).getUltimoRaio()
					+ ((DesenharCirculo) proximo).getRaio());
		}

		// Anda o raio do circulo anterior + (o raio da circunferência sobreescrita no
		// poligono - metade do seu lado)
		if (ultimo instanceof DesenharCirculo && proximo instanceof DesenharPoligono
				&& ((DesenharPoligono) proximo).getNumLados() > 3) {
			return ((int) (Math.ceil(((DesenharCirculo) ultimo).getUltimoRaio()
					+ ((DesenharPoligono) proximo).getCircleRadius()
					- (((DesenharPoligono) proximo).getLado() / 2))));
		}

		// Anda o raio do próximo circulo + (o raio da circunferência sobreescrita no
		// poligono anterior - metade do seu lado)
		if (ultimo instanceof DesenharPoligono && ((DesenharPoligono) ultimo).getUltNumLados() > 3
				&& proximo instanceof DesenharCirculo) {
			return ((int) (Math.ceil(((DesenharCirculo) proximo).getRaio()
					+ ((DesenharPoligono) ultimo).getUltCircleRadius()
					- (((DesenharPoligono) ultimo).getUltLado() / 2))));
		}

		// Anda o raio da circunferência sobreescrita no poligono anterior - metade do
		// seu lado
		if (ultimo instanceof DesenharQuadrado && proximo instanceof DesenharPoligono
				&& ((DesenharPoligono) proximo).getNumLados() > 3) {
			return ((int) (Math.ceil(((DesenharPoligono) proximo).getCircleRadius()
					- (((DesenharPoligono) proximo).getLado() / 2))));
		}
		...
		if (ultimo instanceof DesenharPoligono && ((DesenharPoligono) ultimo).getUltNumLados() > 3
				&& proximo instanceof DesenharQuadrado) {
			return ((int) (Math.ceil(((DesenharPoligono) ultimo).getUltCircleRadius()
					- (((DesenharPoligono) ultimo).getUltLado() / 2))));
		}
		return 0;
	}
